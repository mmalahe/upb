...
function instantRunTourney() {
    ...
    for (currentRound=0; currentRound < rounds; currentRound++) {
        ...
        for (rCounter=0; rCounter < 10; rCounter++) {
            runRound();
        }
    }
...
function loadStateFromString(stateString) {
    ...
    var stateContents = stateString;
    
    // Note there is actually no need to parse it, as that is done automatically.
    // Leaving this here, as uncertain when that can be guaranteed.
    // var stateContents = JSON.parse(stateString); 
    
    var loadGame = stateContents.saveGame;
    var loadProjectsUses = stateContents.saveProjectsUses;
    var loadProjectsFlags = stateContents.saveProjectsFlags;
    var loadProjectsActive = stateContents.saveProjectsActive;
    var loadStratsActive = stateContents.saveStratsActive;
